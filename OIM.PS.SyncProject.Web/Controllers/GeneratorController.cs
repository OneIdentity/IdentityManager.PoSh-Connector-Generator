...
public class GeneratorController : Controller
{
	...
	public IActionResult AddClass(AddClassModel model)
	{
		...
		var addError = ValidateClassProps(props);
		if (addError != null)
		{
	...
	private static string? ValidateClassProps(List<GenClassProp> props)
	{
		if (props.Count < 2)
			return "Class must contain more than one property.";

		if (!props.Exists(q => q.IsPrimaryKey) && !props.Exists(q => q.IsCombinedPrimaryKey))
			return "At least one property must be marked as Primary Key or Combined Primary Key.";

		if (props.Exists(q => q.IsCombinedPrimaryKey) && props.Exists(q => q.IsPrimaryKey))
			return "Can't contain both 'Primary Key' and 'Combined Primary Key' fields.";

		if (props.Exists(q => q.IsCombinedPrimaryKey) && props.Count(q => q.IncludeInCombinedPrimaryKey) < 2)
			return "If a field is marked as Combined PK, at least 2 fields must be marked as \"Include in PK\".\nIf only one field needs to be included in PK then just mark it as PK.";
	...
	public IActionResult EditClass(int index, AddClassModel model)
	{
		...
		var editError = ValidateClassProps(props);
		if (editError != null)
		{
		...
		try
		{
			...
			using (var archive = new ZipArchive(memoryStream, ZipArchiveMode.Create, leaveOpen: true))
			{
				...
				string psModule = new PowerShellModuleGenerator(meta).GenerateModule(tempFolder);
					...
					string psTest = new PowerShellTestGenerator(meta).GenerateTestScript();
					AddZipEntry(archive, $"{ns}_TEST.ps1", psTest);
				...
				if (generateSolution)
					{
						...
						AddZipEntry(archive, $"{ns}.sln", slnContent);

						// Library project
						string libCsproj = GenerateLibraryCsproj(ns);
						AddZipEntry(archive, $"{libProject}/{libProject}.csproj", libCsproj);
						AddZipEntry(archive, $"{libProject}/{ns}.cs", netClass);
						AddZipEntry(archive, $"{libProject}/{ns}Implement.cs", netClassImpl);

						// Test (console) project
						string testCsproj = GenerateConsoleCsproj(ns, libProject);
						AddZipEntry(archive, $"{testProject}/{testProject}.csproj", testCsproj);
						AddZipEntry(archive, $"{testProject}/{ns}_TEST.cs", netTestClass);

						// Additional files
						AddZipEntry(archive, $"{ns}Metadata.json", metaJson);
						AddZipEntry(archive, $"{libProject}/{ns}_NET.xml", xmlNet);
